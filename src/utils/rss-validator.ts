...
/**
 * Get a human-readable description of the validation error type
 */
export function getErrorTypeDescription(errorType?: ValidationErrorType): string {
  switch (errorType) {
    case 'NOT_FOUND':
      return 'URL does not exist (404)';
    case 'HTTP_ERROR':
      return 'Server error (non-200 response)';
    case 'NOT_XML':
      return 'Not XML content (likely HTML page)';
    case 'NOT_RSS':
      return 'Valid XML but not RSS/Atom feed';
    case 'MALFORMED':
      return 'RSS feed structure is invalid';
    case 'TIMEOUT':
...
/**
 * Validates multiple RSS feed URLs in parallel
 */
export async function validateRssFeeds(urls: string[]): Promise<FeedValidationResult[]> {
  // Deduplicate URLs
  const uniqueUrls = [...new Set(urls)];

  // Validate all feeds in parallel with a reasonable concurrency limit
  const BATCH_SIZE = 5;
  const results: FeedValidationResult[] = [];

  for (let i = 0; i < uniqueUrls.length; i += BATCH_SIZE) {
    const batch = uniqueUrls.slice(i, i + BATCH_SIZE);
    const batchResults = await Promise.all(batch.map(validateRssFeed));
